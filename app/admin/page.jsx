...
export default function AdminPage() {
	...
	const [activeTab, setActiveTab] = useState("projects"); // 'projects' | 'experiences' | 'education' | 'skills' | 'profile'
	...
	const handleFileUpload = async (e, targetField = "thumbnail") => {
		...
		try {
			...
			reader.onload = async () => {
				...
				if (targetField === "thumbnail") setActiveProject((prev) => ({ ...prev, thumbnail: result.url }));
			};
	...
	return (
		<main className="min-h-screen bg-zinc-950 text-zinc-200 font-sans selection:bg-zinc-800">
			...
			<div className="max-w-6xl mx-auto px-6 py-8">
				{/* Tab Navigation */}
				<div className="flex items-center gap-2 border-b border-zinc-900 pb-4 mb-8 overflow-x-auto">
					<button onClick={() => setActiveTab("projects")} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-medium transition-all whitespace-nowrap ${activeTab === "projects" ? "bg-zinc-800 text-zinc-100 border border-zinc-700/80" : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/50"}`}>
						<FontAwesomeIcon icon={faFolderOpen} className="text-xs" /><span>Projects ({projects.length})</span>
					</button>

					<button onClick={() => setActiveTab("experiences")} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-medium transition-all whitespace-nowrap ${activeTab === "experiences" ? "bg-zinc-800 text-zinc-100 border border-zinc-700/80" : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/50"}`}>
						<FontAwesomeIcon icon={faBriefcase} className="text-xs" /><span>Pengalaman ({experiences.length})</span>
					</button>

					<button onClick={() => setActiveTab("education")} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-medium transition-all whitespace-nowrap ${activeTab === "education" ? "bg-zinc-800 text-zinc-100 border border-zinc-700/80" : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/50"}`}>
						<FontAwesomeIcon icon={faGraduationCap} className="text-xs" /><span>Pendidikan ({achievements.length})</span>
					</button>

					<button onClick={() => setActiveTab("skills")} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-medium transition-all whitespace-nowrap ${activeTab === "skills" ? "bg-zinc-800 text-zinc-100 border border-zinc-700/80" : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/50"}`}>
						<FontAwesomeIcon icon={faCode} className="text-xs" /><span>Skills ({skills.length})</span>
					</button>

					<button onClick={() => setActiveTab("profile")} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-medium transition-all whitespace-nowrap ${activeTab === "profile" ? "bg-zinc-800 text-zinc-100 border border-zinc-700/80" : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/50"}`}>
				...
				{activeTab === "projects" && (
					<div>
						<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
							...
							<div className="flex items-center gap-2.5">
								<button onClick={() => { setActiveProject({ show: true, title: "", desc: ["", ""], year: new Date().getFullYear().toString(), preview: "", code: "", thumbnail: "", images: [], tech: [], slug: "", category: [1], _index: -1 }); setIsEditingProject(true); }} className="text-xs font-medium bg-zinc-100 text-zinc-950 px-3.5 py-2 rounded-lg flex items-center gap-1.5">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
							{projects.map((proj, idx) => (
								<motion.div key={proj.slug || idx} layout className={`bg-zinc-900/60 border rounded-xl p-4 flex flex-col justify-between ${proj.show ? "border-zinc-800/80" : "border-zinc-900/60 opacity-50 bg-zinc-950/40"}`}>
									<div>
										...
										{proj.thumbnail ? (
											<div className="w-full h-32 bg-zinc-950 rounded-lg overflow-hidden mb-3 border border-zinc-800/60"><img src={proj.thumbnail} alt={proj.title} className="w-full h-full object-cover opacity-90" /></div>
										) : (
									...
									<div className="pt-3 border-t border-zinc-800/60 flex items-center justify-between text-xs">
										...
										<div className="flex items-center gap-1.5">
											<button onClick={() => { setActiveProject({ ...proj, _index: idx }); setIsEditingProject(true); }} className="p-1.5 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 rounded"><FontAwesomeIcon icon={faEdit} className="text-xs" /></button>
						...
						<div className="space-y-4">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
								{achievements.map((ach, idx) => (
									<div key={ach.id || idx} className="bg-zinc-900/60 border border-zinc-800/80 rounded-xl p-4 flex items-center justify-between gap-3">
				...
				{activeTab === "skills" && (
					<div className="space-y-6">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							{skills.map((sk, idx) => (
								<div key={sk.key || idx} className="bg-zinc-900/60 border border-zinc-800/80 rounded-xl p-5 space-y-3">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							...
							<div className="bg-zinc-900/60 border border-zinc-800/80 rounded-xl p-6 space-y-4">
								...
								<div>
									...
									<input type="text" value={profile.name || ""} onChange={(e) => setProfile({ ...profile, name: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none" />
								</div>

								<div>
									<label className="block text-[11px] font-medium text-zinc-400 mb-1">Nama Panggilan / Brand</label>
									<input type="text" value={profile.nickname || ""} onChange={(e) => setProfile({ ...profile, nickname: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none" />
								</div>

								<div>
									<label className="block text-[11px] font-medium text-zinc-400 mb-1">Headline Utama (Role)</label>
									<input type="text" value={profile.headline || ""} onChange={(e) => setProfile({ ...profile, headline: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none" />
								</div>

								<div>
									<label className="block text-[11px] font-medium text-zinc-400 mb-1">Sub-headline (Spesialisasi)</label>
									<input type="text" value={profile.subheadline || ""} onChange={(e) => setProfile({ ...profile, subheadline: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none" />
								</div>

								<div>
									<label className="block text-[11px] font-medium text-zinc-400 mb-1">Domisili / Lokasi</label>
									<input type="text" value={profile.location || ""} onChange={(e) => setProfile({ ...profile, location: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none" />
								</div>
							</div>

							{/* Right Form: Quote & Social Media */}
							<div className="space-y-6">
								<div className="bg-zinc-900/60 border border-zinc-800/80 rounded-xl p-6 space-y-4">
									<h3 className="text-xs font-semibold text-zinc-200 uppercase tracking-wider flex items-center gap-2">
									...
									<div>
										<label className="block text-[11px] font-medium text-zinc-400 mb-1">Quote Baris 1 (Teks Hitam)</label>
										<input type="text" value={profile.quoteLine1 || ""} onChange={(e) => setProfile({ ...profile, quoteLine1: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none" />
									</div>

									<div>
										<label className="block text-[11px] font-medium text-zinc-400 mb-1">Quote Baris 2 (Teks Abu-abu)</label>
										<input type="text" value={profile.quoteLine2 || ""} onChange={(e) => setProfile({ ...profile, quoteLine2: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none" />
									</div>
								</div>
								...
								<div className="bg-zinc-900/60 border border-zinc-800/80 rounded-xl p-6 space-y-4">
									...
									<div>
										<label className="block text-[11px] font-medium text-zinc-400 mb-1">Email Kontak</label>
										<input type="email" value={profile.email || ""} onChange={(e) => setProfile({ ...profile, email: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none" />
									</div>

									<div>
										<label className="block text-[11px] font-medium text-zinc-400 mb-1">URL GitHub</label>
										<input type="text" value={profile.githubUrl || ""} onChange={(e) => setProfile({ ...profile, githubUrl: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none font-mono" />
									</div>

									<div>
										<label className="block text-[11px] font-medium text-zinc-400 mb-1">URL LinkedIn</label>
										<input type="text" value={profile.linkedinUrl || ""} onChange={(e) => setProfile({ ...profile, linkedinUrl: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none font-mono" />
									</div>

									<div>
										<label className="block text-[11px] font-medium text-zinc-400 mb-1">URL Resume PDF</label>
										<input type="text" value={profile.resumeUrl || ""} onChange={(e) => setProfile({ ...profile, resumeUrl: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none font-mono" />
									</div>
								</div>
							</div>
						</div>
			...
			{/* EDIT MODALS (Project, Exp, Ach, Skill) */}
			<AnimatePresence>
					...
					<div className="fixed inset-0 z-50 bg-zinc-950/80 backdrop-blur-sm flex items-center justify-center p-4">
						<motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
							...
							<form onSubmit={handleSaveProjectForm} className="space-y-4">
								...
								<div><label className="block text-[11px] font-medium text-zinc-400 mb-1">Deskripsi Singkat</label><textarea rows={2} value={Array.isArray(activeProject.desc) ? activeProject.desc[0] || "" : activeProject.desc} onChange={(e) => { const descArray = Array.isArray(activeProject.desc) ? [...activeProject.desc] : [activeProject.desc, ""]; descArray[0] = e.target.value; setActiveProject({ ...activeProject, desc: descArray }); }} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none" /></div>
								...
								<div className="bg-zinc-950/60 border border-zinc-800 p-3.5 rounded-lg space-y-2.5">
									...
									<div className="flex items-center gap-2"><input type="text" value={activeProject.thumbnail || ""} onChange={(e) => setActiveProject({ ...activeProject, thumbnail: e.target.value })} className="flex-1 bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-1.5 text-xs font-mono text-zinc-300 outline-none" /><label className="flex items-center gap-1.5 bg-zinc-800 hover:bg-zinc-700 text-xs text-zinc-200 px-3 py-1.5 rounded-lg border border-zinc-700 cursor-pointer"><FontAwesomeIcon icon={isUploading ? faSpinner : faUpload} className={`text-[11px] ${isUploading ? "animate-spin" : ""}`} /><span>Upload</span><input type="file" accept="image/*" onChange={(e) => handleFileUpload(e, "thumbnail")} className="hidden" /></label></div>
								</div>
								<div className="pt-3 border-t border-zinc-800 flex justify-end gap-2"><button type="button" onClick={() => setIsEditingProject(false)} className="px-3.5 py-1.5 bg-zinc-800 text-zinc-300 rounded-lg text-xs">Batal</button><button type="submit" className="px-4 py-1.5 bg-zinc-100 text-zinc-950 font-medium rounded-lg text-xs">Simpan Project</button></div>
							</form>
						</motion.div>
					</div>
				)}
